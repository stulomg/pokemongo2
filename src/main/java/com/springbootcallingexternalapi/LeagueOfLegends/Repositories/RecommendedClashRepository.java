...
public class RecommendedClashRepository {
    ...
    public List<RecommendedClashRoleModel> recommendedRole(ArrayList accounts) throws NoDataException {
        ...
        List<RecommendedClashRoleModel> recommendedClashRoleModel = jdbcTemplate.query(sql, params, BeanPropertyRowMapper.newInstance(RecommendedClashRoleModel.class));
    ...
    public List<RecommendedClashChampionModel> recommendChampion(ArrayList accounts) throws NoDataException {
        String sql = "SELECT account,champion ,COUNT(champion)as gamesplayed,MAX(\"championPoints\")as maxmastery\n" +
                "FROM \"MatchHistory\" \n" +
                "WHERE account in (?,?,?,?,?)\n" +
                "GROUP BY account,champion \n" +
                "ORDER BY account,champion;";
        Object[] params = {accounts.get(0), accounts.get(1), accounts.get(2), accounts.get(3), accounts.get(4)};
        List<RecommendedClashChampionModel> recommendedClashChampionModel = jdbcTemplate.query(sql, params, BeanPropertyRowMapper.newInstance(RecommendedClashChampionModel.class));
    ...
    public List<RecommendedClashWinRateModel> championWin(ArrayList accounts) throws NoDataException {
        String sql = "SELECT account, champion ,COUNT(\"win\")as win\n" +
                ...
                "ORDER BY account, champion";
        Object[] params = {accounts.get(0), accounts.get(1), accounts.get(2), accounts.get(3), accounts.get(4)};
        List<RecommendedClashWinRateModel> recommendedClashWinModel = jdbcTemplate.query(sql, params, BeanPropertyRowMapper.newInstance(RecommendedClashWinRateModel.class));
