...
public class MostPopularRepository {
  ...
  public List<MostPopularModel> popularAccount() throws NoDataException {
    String sql =
        "SELECT  \"account\", \"champion\", (SELECT TO_CHAR(\"date\"::date,'Mon dd') AS date FROM  \"LeagueHistory\" WHERE \"account\" = (SELECT \"account\" FROM  \"LeagueHistory\" GROUP BY \"account\" ORDER BY COUNT( \"account\" ) DESC LIMIT 1) GROUP BY \"date\"::date , \"account\" ORDER BY COUNT( \"date\" ) DESC LIMIT 1)\n"
            +
            "                FROM  \"MasteryHistory\"\n" +
