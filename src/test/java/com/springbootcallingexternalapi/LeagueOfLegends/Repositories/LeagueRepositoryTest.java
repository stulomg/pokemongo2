...
public class LeagueRepositoryTest {
  ...
  @Test
  void divisionHistoryDefaultCase()
      throws CharacterNotAllowedException, AccountDataException, InterruptedException, LeagueDataNotFoundException {
    for (int i = 0; i < 21; i++) {
  ...
  @Test
  void divisionHistoryLessThanTwentyDefaultCase()
      throws CharacterNotAllowedException, AccountDataException, InterruptedException, LeagueDataNotFoundException {
    for (int i = 0; i < 15; i++) {
      Timestamp timestamp = new Timestamp(System.currentTimeMillis());
      Thread.sleep(100);
  ...
  void accountNotFoundExceptionDivisionHistory() {
    Assertions.assertThrows(LeagueDataNotFoundException.class,
        () -> repository.divisionHistory("testtres", 3));
  }
  ...
  void characterNotAllowedDivisionHistory() {
    ...
    Assertions.assertThrows(CharacterNotAllowedException.class,
        () -> repository.divisionHistory("te*st2", 2));
  }
  ...
  void insertLeagueInfoSuccessfullyDefaultCase() throws AccountDataException {
    ...
    //THEN
    List<LeagueInfoModel> resultSet = jdbcTemplate.query("SELECT * FROM \"LeagueHistory\"",
  ...
  @Test
  void accountDataExceptionInsertLeagueInfo()
  ...
  @Test
  void maxDivisionDefaultCase()
      throws CharacterNotAllowedException, AccountDataException, CharacterNotAllowedExceptionOwner, OwnerNotFoundException {
//GIVEN
...
//THEN
    List<MaxDivisionModel> resultSet = repository.getMaxDivision(infoModel.getOwner(),
        infoModel2.getOwner(), 1, 2);
    Assertions.assertEquals(1, resultSet.size());
  ...
  void OwnerNotFoundExceptionMaxDivision() {
    Assertions.assertThrows(OwnerNotFoundException.class,
        () -> repository.getMaxDivision("Owner", "Owner", 1, 1));
  }

  @Test
  void characterNotAllowedExceptionMaxDivision() {
    Assertions.assertThrows(CharacterNotAllowedExceptionOwner.class,
        () -> repository.getMaxDivision("O*ner", "O*ner", 3, 3));
  }
}