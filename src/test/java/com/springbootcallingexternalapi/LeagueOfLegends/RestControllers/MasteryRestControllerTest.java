...
class MasteryRestControllerTest {
    ...
    void accountMasteryHistory() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(get("/account/masteryHistory/testuno").header("authorization", token)).andExpect(status().isOk()).andReturn();
        String response = mvcResult.getResponse().getContentAsString();
        MasteryHistoryModel[] masteryHistory = new ObjectMapper().readValue(response, MasteryHistoryModel[].class);
