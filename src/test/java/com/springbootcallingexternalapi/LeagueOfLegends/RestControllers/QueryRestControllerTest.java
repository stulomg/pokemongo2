...
class QueryRestControllerTest {
  ...
  void newQueryDefaultCase() throws Exception {
    ...
    List<QueryModel> resultSet = jdbcTemplate.query("SELECT * FROM \"SpecificQuery\"",
  ...
  void newQuerySyntaxErrorExceptionNewQuery() throws Exception {
    ...
    mockMvc.perform(MockMvcRequestBuilders.post("/loldata/newquery").header("authorization", token)
        ...
        .andExpect(content().string(
            "Error in the syntax of the query : test prueba, check the data entered"));
  }
  ...
  void newQueryInvalidParameterExceptionNewQuery() throws Exception {
    ...
    mockMvc.perform(MockMvcRequestBuilders.post("/loldata/newquery").header("authorization", token)
        ...
        .andExpect(status().isBadRequest())
        .andExpect(content().string(
            "Invalid parameter in the query : test prueba, check the data entered"));
  }
  ...
  void newQueryQueryCriteriaExistException() throws Exception {
    ...
    mockMvc.perform(MockMvcRequestBuilders.post("/loldata/newquery").header("authorization", token)
        ...
        .andExpect(status().isBadRequest())
        .andExpect(
            content().string("The criteria: test, is already registered, please use another name"));
  }
  ...
  void listQueryDefaultCase() throws Exception {
    ...
    MvcResult mvcResult = mockMvc.perform(
            MockMvcRequestBuilders.get("/loldata/listquery").header("authorization", token))
  ...
  void queryDefaultCase() throws Exception {
    ...
    String token = securityUserService.generateToken();
    MvcResult mvcResult = mockMvc.perform(
            MockMvcRequestBuilders.get("/loldata/query/test").header("authorization", token))
        .andExpect(status().isOk()).andReturn();
    String response = mvcResult.getResponse().getContentAsString();
    QueryResponseModel[] recommendedRole = new ObjectMapper().readValue(response,
  ...
  void queryQuerySyntaxErrorException() throws Exception {
    ...
    mockMvc.perform(
        ...
        .andExpect(status().isBadRequest())
        .andExpect(
            content().string("Error in the syntax of the query : test, check the data entered"));
  }
}