...
class RecommendedClashRestControllerTest {
  ...
  void getRecommendedRoleDefaultCase() throws Exception {
    ...
    List<RecommendedClashDataModel> participants = Stream.of(
    ...
    MvcResult mvcResult = mockMvc.perform(
            MockMvcRequestBuilders.get("/loldata/clash/recommended").header("authorization", token)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(participants)))
        .andExpect(status().isOk()).andReturn();
    String response = mvcResult.getResponse().getContentAsString();
    RecommendedClashResponseModel[] recommendedRole = new ObjectMapper().readValue(response,
        RecommendedClashResponseModel[].class);
